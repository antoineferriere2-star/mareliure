...
**Agent :** Codex (GPT-6) — 22 septembre 2026, `fix/binder-account-activation`.

- Diagnostic production : `contact@metre-pro.fr` (adresse confirmée par l'utilisateur et confirmée dans Supabase) possède une invitation `pending` pour l'atelier approuvé `r r`, valide jusqu'au 29 septembre, mais aucune ligne `marketplace_binder_members`. La connexion générique envoyait donc ce compte vers `/mes-livres`.
- Correctif : recherche serveur des seules invitations en attente pour l'adresse **confirmée** du compte authentifié, page `/activer-mon-atelier` avec action explicite, acceptation contrôlée par adresse et invitation, puis rattachement actif et événements d'audit existants. La connexion priorise l'invitation avant l'espace client. Un choix « Atelier partenaire » reste dans le retour du lien e-mail et mène à une explication si aucune invitation n'existe.
- Aucune migration. Tests ciblés du routage, du retour OTP et de l'acceptation par adresse confirmée verts. Typecheck, lint ciblé, build Ma Reliure et suite complète (2 881 tests) verts. Prévisualisation navigateur locale indisponible (`vite preview` cherche `dist/server/server.js` alors que le build Nitro produit `.output` ; `vite dev` n'a pas répondu avant le délai) ; faire la QA navigateur sur le Worker publié.
- Reste : CI de la PR, fusion/déploiement, activation de l'invitation réelle du compte confirmé et vérification du rattachement, puis QA production de l'arrivée dans `/atelier`.

## Latest handoff

**Agent :** Codex (GPT-6) — 22 septembre 2026, `fix/binder-account-activation`.

- Retour utilisateur : une autre adresse connectée tombe aussi sur l'espace client. Le parcours relieur doit permettre de créer directement un atelier, sans attendre une invitation, tout en gardant l'autorisation des leads dans l'admin.
- Sur `/auth?space=atelier`, le lien/code e-mail est le chemin principal et conserve l'intention « atelier ». Après vérification de l'adresse, `/activer-mon-atelier` permet soit d'accepter une invitation existante, soit de créer son atelier. Cette création inscrit un atelier `pending_review` et un membre OWNER actif uniquement pour ce **nouvel atelier** ; elle ne récupère jamais un atelier approuvé existant ni un accès désactivé. L'atelier peut préparer prestations, contacts et devis. L'admin voit l'atelier dans Relieurs et décide séparément de l'approuver ; seul `approved` permet les leads.
- Les contrôles serveur écartent tout atelier non approuvé de la liste/détail des leads, des réponses aux offres, des conversations et décisions, des aperçus de messages et de l'import d'un dossier dans les ouvrages, y compris pour un ancien match sélectionné. Aucun changement de schéma ni de données de production.
- Vérifications locales : `npm run typecheck` vert, `npm run lint` vert (13 avertissements historiques), `npm run build:mareliure` vert avec vérification du projet Supabase du bundle, et `npm test -- --testTimeout 30000` : 2 885 tests verts. Un premier passage de tests en parallèle avec build/lint avait trois timeouts de scans du dépôt, tous passés lors de la réexécution isolée. Browser QA locale via `vite dev` : `/auth?space=atelier` affiche directement l'onglet relieur, le lien/code et le contrôle admin ; `/partenaires-relieurs` pointe clairement vers la création de l'espace. Le formulaire post-connexion n'a pas pu être exercé en navigateur sans créer de compte en production. `vite preview` échoue sur le chemin `dist/server/server.js` alors que Nitro produit `.output`; `wrangler` n'est pas installé localement.
- PR #17 mise à jour : https://github.com/antoineferriere2-star/mareliure/pull/17, SHA `eccdadf7df906f57e8cd695334650e0e343d242e`, mergeable, CI `quality` verte sur ce SHA. La fusion de cette PR a été refusée par l'auto-review lors du tour précédent, faute d'autorisation explicite pour ce merge ; une attribution directe OWNER de l'atelier existant `r r` à `contact@metre-pro.fr` a également été refusée. Aucune de ces actions n'a été effectuée. Demander l'autorisation de fusion/déploiement du **nouveau** périmètre, puis faire la QA navigateur réelle et production.

## Latest handoff

**Agent :** Codex (GPT-6) — 23 septembre 2026, `main`.

- L'utilisateur a explicitement autorisé la fusion de PR #17, le déploiement et la QA production. PR #17 fusionnée par merge commit normal `c6f0a524b17350021a4fae31c28ed1fbe68d1cf5` ; branche `fix/binder-account-activation` conservée ; `main` fast-forward sur ce SHA. CI `quality` verte au head `5289c36a4165e1aa037b05d6bbea30ddb326b8ef` avant fusion.
- Un seul déploiement depuis `main` via `npm run deploy:mareliure` ; build contrôlé sur Supabase `hljxohondjvrkzqicexl`, bundle client et bundle serveur vérifiés ; Worker `mareliure` version `1d5ee208-601b-44df-8b63-7770e2172e09`, confirmée active à 100 % par Wrangler. Aucune migration.
- Smoke production : `/`, `/auth?space=atelier`, `/partenaires-relieurs`, `/candidature-atelier`, `/activer-mon-atelier`, `/admin/ateliers`, `/atelier` répondent HTTP 200. Dans le navigateur connecté en admin, Leads et Ateliers chargent ; le back-office `/marketplace/binders` affiche les ateliers et ses commandes de statut ; `/activer-mon-atelier` affiche le formulaire de création avec la mention de validation admin ; aucune erreur console sur ce formulaire.
- Limite de QA : aucune création de compte ou d'atelier réels n'a été faite en production ; l'activation post-e-mail et le nouveau verrou des leads ont été vérifiés par tests serveur et CI, pas par une inscription E2E en production. L'admin moderne `/admin/ateliers` affiche les statuts mais l'action Approuver/Suspendre est dans l'ancien back-office `/marketplace/binders` ; un lien ou une unification des écrans améliorerait la découverte de cette commande. Aucune donnée métier n'a été modifiée par cette QA, hors éventuelle télémétrie de consultation.

## Latest handoff

**Agent :** Codex (GPT-6) — 23 septembre 2026, `main`, après recette E2E production.

- Complément au handoff précédent : recette E2E sur `https://mareliure.aferriere.workers.dev` avec compte Auth **QA temporaire**, adresse confirmée par l'API admin, mot de passe temporaire, sans toucher à la session admin de `mareliure.fr`. Connexion avec `?space=atelier` → `/activer-mon-atelier` (et non espace client) → création `Atelier QA Codex temporaire` → `/atelier`. Le bandeau « Accès aux leads non autorisé » est visible ; Leads ne montre aucun dossier ; Devis est accessible et vide. Dans l'admin production `/marketplace/binders`, l'atelier apparaît `À valider` avec bouton `Approuver` non actionné : 3 approuvés sur 4 pendant la recette.
- Nettoyage vérifié : l'atelier QA n'avait qu'un membre et zéro contact, ouvrage, devis, facture, match, événement ou prestation. Atelier et compte Auth QA supprimés ; l'admin revient à 3 approuvés sur 3. Aucun atelier réel ni lead n'a été modifié.
- Limite persistante : la remise d'un vrai lien/code par e-mail et la connexion avec l'adresse personnelle du relieur n'ont pas été testées ; la recette utilise un compte confirmé et un mot de passe QA. Le contrôle admin se trouve dans `/marketplace/binders`, distinct de la liste moderne `/admin/ateliers` ; rendre l'action plus visible dans cette dernière peut être traité ensuite.

---

## Latest handoff

**Agent :** Codex (GPT-6) — 23 septembre 2026, `feat/ux-design-polish`.

- Espace relieur : navigation bureau/mobile clarifiée, accueil recentré sur les actions, listes Leads/Messages/Devis/Ouvrages/Contacts et fiche dossier harmonisées avec des états vides explicites et une lecture éditoriale plus compacte. Le statut `pending_review` reste visible et les autorisations métier sont inchangées.
- Quote Workbench : palette à clics rapides dans l'ordre Favoris → Récentes → Fréquentes → Catégories → Recherche → Ligne libre ; TVA globale avec taux atelier et surcharge par devis ; lignes compactes avec description client optionnelle ; total mobile collant. Les PDF masquent les références tarifaires internes et évitent la répétition de la TVA sur chaque ligne lorsqu'elle est uniforme.
- Paramètres : identité documentaire, TVA par défaut, pied de devis et aperçu direct regroupés sous « Devis & documents ». Fine Bindery reçoit une identité crème, vert profond et laiton, ainsi qu'un discours de conciergerie distinct de Ma Reliure.
...
- Migration additive `20260923120000_marketplace_invoice_compliance.sql`, non appliquée : profil juridique atelier, identité et adresses client, snapshots réglementaires, dates de prestation/échéance, mentions figées, champs futurs de transmission, conservation sur dix ans et avoir complet séparé. Les anciennes factures restent émises sans réécriture de leurs montants ou identités. Les fonctions d'émission et d'avoir sont réservées au rôle serveur ; RLS deny-all côté navigateur.
- Les PDF facture affichent identité juridique, client facturé, date de prestation, échéance, ventilation multi-taux, mentions B2B conditionnelles, option sur les débits et IBAN. QA locale : facture simple 1 page et facture longue 42 lignes / 3 pages, toutes rendues avec Poppler et inspectées visuellement ; aucune provenance interne affichée.
...
**Agent :** Codex (GPT-6) — 23 septembre 2026, livraison production depuis `main`.

- PR #19 à #25 fusionnées par merge commits normaux, branches conservées. Les merges de cette séquence sont `c640f02c833e55b39655f7f2fb5569c559802b09`, `ece28205f4bacd75c42003ac7c1a0440ae120109`, `497ff48b0a98e7a80d35c231f4e5d4a6de657d6d`, `a0626f685ad88d06cfd4f6e82d006171f42154b5`, `9f2c97f8162bd12e6ab972edf420b4d2a19f61f8`, `544913f9940a3530f74e0ea4214a5346328fd134` et `ee8918191cebf1b0c4c9be2aaacffbec6b215875`. Toutes les CI de PR sont vertes ; la dernière couvre 211 fichiers et 2 925 tests, typecheck, lint et build.
- Production Supabase `hljxohondjvrkzqicexl` alignée avec les migrations `20260923100000_marketplace_quote_size_blocks_photos.sql`, `20260923110000_marketplace_document_branding.sql`, `20260923120000_marketplace_invoice_compliance.sql` et `20260923130000_marketplace_binder_pricing_catalog.sql`. Deux défauts de migration ont été corrigés dans les PR #23 et #24 avant reprise ; chaque tentative en erreur a été annulée par PostgreSQL.
- Comptes agrégés avant/après inchangés : 1 profil documentaire, 2 contacts, 2 devis, 12 lignes de devis, 2 factures, 12 lignes de facture et 2 compteurs. Les nouvelles tables photo, avoir et préférences tarifaires sont vides. Les seules évolutions sur les pièces historiques sont les métadonnées techniques prévues : clés/blocs de ligne et classement réglementaire. Les types Supabase ont été régénérés depuis les schémas `public,graphql_public` de production et validés par le typecheck.
- Un seul déploiement applicatif a été exécuté depuis `main` au SHA `ee8918191cebf1b0c4c9be2aaacffbec6b215875`. Worker `mareliure` version `7549eac2-081a-4459-bcb4-0c0161029695`, active à 100 %. Smoke : `/`, `/auth?space=atelier`, `/atelier`, `/atelier/tarifs`, `/atelier/devis`, `/atelier/factures` et `/admin/ateliers` répondent 200 ; les fonctions serveur observées répondent `Ok`, sans 500.
- QA production admin : les deux onglets du référentiel chargent ; la base affiche 45/45 prestations, 41 prix numériques et 4 sur étude, version `mareliure-base-prices-v1`. Contrôle direct : 16 modes forfait, 17 unités, 8 « à partir de », 4 « sur étude », unités présentes partout. Recherche « patrimoniale » correcte ; filtres « Tarifs numériques » (41) et « Sur étude » (les quatre clés attendues) corrects. Le tableau reste contenu dans un défilement horizontal interne sur le viewport mobile, sans débordement de page. L'onglet Observations ateliers affiche son état vide attendu.
- Le compte admin disponible ne possède pas d'atelier actif. La QA authentifiée de `/atelier/tarifs` avec l'atelier réel n'a donc pas été rejouée dans ce dernier passage ; l'appartenance active du compte atelier connu et les 45 lignes de base ont été vérifiées en lecture seule. La création d'un nouveau compte QA ou d'un lien de connexion a été refusée par l'auto-review, donc aucune donnée d'accès n'a été créée ni exportée.

---

## Latest handoff

**Agent :** Codex (GPT-6) — 23 septembre 2026, `feat/finebindery-network-phase1`.

- Phase 1 FineBindery Network : profil professionnel structuré dans l'espace atelier, spécialités existantes, langues et sélections du référentiel `reliure-fr-v1`, identité publique, publication explicite et slug stable. L'atelier peut créer une réalisation indépendante ou la rattacher à l'un de ses ouvrages, ajouter les photos avant/après, puis la publier seulement après consentement explicite.
- Pages publiques : annuaire `/professionnels` avec filtres sans classement et page éditoriale `/fr/:slug` avec SEO, canonical, OpenGraph et données structurées. Seuls les ateliers approuvés et publiés apparaissent ; les profils absents sont `noindex`. La projection publique du portfolio exclut identifiants d'atelier, ouvrage source, chemins de stockage, consentement et dates internes.
- Demande directe : le CTA transporte le slug et la provenance `FINEBINDERY_PROFILE`; le serveur accepte cette attribution seulement si la page est encore publiée, crée une seule affectation à cet atelier et rend la provenance visible dans la liste et le détail du dossier. La règle d'exclusivité interdit l'envoi à un autre atelier.
- Migration additive `20260923140000_finebindery_public_profiles.sql`, non appliquée : champs publics sur `marketplace_binders`, consentement/publication et rattachement ouvrage sur le portfolio, index sélectifs, provenance étendue et garde base de données contre le rattachement à l'ouvrage d'un autre atelier. Les profils et réalisations existants restent privés par défaut ; RLS et bucket privé restent inchangés.
- QA locale avec données de recette sans écriture distante : page publique et deux réalisations inspectées en bureau et à 375/390/430 px, CTA visible, aucun débordement ; annuaire et filtres contrôlés ; éditeur contrôlé en bureau et à 390 px. Une barre d'actions qui masquait des champs sur mobile a été corrigée. Captures conservées hors dépôt dans le dossier de visualisations de la tâche.
...
- **Mise à jour après arbitrage du propriétaire, PR #47 (`89d1d5e`, 25 septembre 2026) :** les six captures de #46 sous `/product/*.webp` ne sont plus publiées ; leurs réponses 404 sont volontaires. `WorkshopProductInvite` a été supprimé. La version #47 et ses captures sous `/photos/product/` font référence ; ne pas réintroduire ces fichiers ni un second bloc atelier.

---

## Latest handoff

**Agent :** Codex — 25 septembre 2026, réception de la passation Claude PR #47.

- Périmètre de cette reprise : synchronisation de main et documentation uniquement. PR #47 confirmée fusionnée sur `89d1d5e`. Worker `43dce951` et smoke test vert annoncés dans la passation Claude ; aucun nouveau déploiement par Codex pendant cette reprise.
- Arbitrage conservé : refonte publique #47, SEO de #46 et variante atelier de LandingHeader. Tout nouveau bouton public utilise `actions.tsx` (`ActionLink` / `actionClass`). Captures : `ProductShot.tsx` et `/photos/product/`, avec données de démonstration et sans exposer la grille interne. La mention Stripe « Facultatif, en préparation » reste obligatoire jusqu’à implémentation effective.
- FineBindery : conserver l’ouverture du réseau en France dans les cinq langues, les dictionnaires complets sans spread anglais, les couleurs via jetons `.fb-site` et le menu mobile natif. Les contrats `publicFront.contract.test.ts` protègent ces choix.
- Coordination : avant tout chantier front, fetch, revue des PR ouvertes ou fusionnées depuis 48 h, lecture de la fin de ce document, puis annonce du périmètre ici avant de coder. La PR documentaire #38 était encore ouverte lors de cette vérification ; ne pas la fusionner aveuglément sur un état plus récent.
- Points ouverts signalés par Claude, non corrigés dans cette reprise : TVA recouverte par Total TTC dans le PDF ; prochaine action de création d’ouvrage sur projet clos ; jargon dans l’en-tête du parcours FineBindery ; poids JavaScript public ; nom accessible du lien d’accueil de BuildPublicShell avant chargement ; aucun profil FineBindery publié pour la recette. Pour cette dernière, utiliser des fixtures temporaires et retirer proprement les modifications générées de routeTree après contrôle de leur diff, sans écraser un changement tiers.

---

## Latest handoff

**Agent :** Codex — 25 septembre 2026, soir, `docs/handoff-public-sites-pdf-20260925`.

- **Périmètre annoncé avant modification : documentation uniquement.** Publier la note locale concernant #47 et consigner #48 ; aucun chantier applicatif, aucune migration, aucun déploiement. Cette PR documentaire doit rester ouverte jusqu’à accord explicite du propriétaire. Aucune fusion ni aucun déploiement ultérieur sans cet accord.
- **Référence vérifiée après fetch :** `origin/main = 50a95619fa152d6c276c9b026204b14428808a4b`, merge de la PR #48. Production au même SHA, Worker `mareliure` version `16903c82` et smoke test vert selon la passation Claude ; pas de nouvelle vérification du Worker dans cette reprise documentaire.
- **PR #48 — PDF devis et factures :** dans `src/marketplace/quotes/documentPdf.ts`, `drawTotals` décalait les lignes de 17 pt mais le bandeau du total occupait de y−9 à y+18. Il recouvrait donc la dernière ligne TVA, ou Total HT en franchise. Le correctif réserve `28 + BAND_GAP` avant un éventuel changement de page, puis retire `BAND_GAP = 12` avant de dessiner le bandeau. Géométrie seulement : aucun montant ni calcul modifié. Ne pas réduire la hauteur du bandeau : le total de 11 pt gras doit conserver sa marge.
- **Régression PDF :** `src/marketplace/quotes/documentPdf.totals.test.ts` observe `PDFPage.prototype.drawText` et `drawRectangle` et vérifie l’absence de texte recouvert : un taux, plusieurs taux avec remise, acompte, franchise. Les quatre cas échouaient avant correction selon Claude. Réutiliser ce procédé pour les futurs changements de mise en page. Les PDF déjà émis ne sont pas régénérés ; seuls les nouveaux exports bénéficient du correctif.
...
**Agent :** Codex — 26 septembre 2026, `fix/public-trust-journeys-seo`.

- Périmètre annoncé avant code : audit ciblé des sites publics Ma Reliure et FineBindery après lecture intégrale de l’audit du 26 septembre, vérification des constats dans les sources, recettes neuves desktop/mobile, cohérence des prix sur photos et après inspection, réseau publié, parcours d’entrée, traductions et SEO. Une seule base de code pour les deux marques. Pas de refonte graphique ni nouvelles prestations génériques.
- Base : origin/main `bbd4b57`, PR #47/#48/#49 fusionnées ; PR documentaire #38 toujours ouverte. Respecter les choix #47, le système de boutons et les cinq dictionnaires complets. Consigner les décisions métier manquantes sans inventer de prix, assurance, délais, frais de retour ou conditions atelier.
- Aucune demande réelle créée, aucune suppression de production. Préparer une PR relisible ; aucune fusion ni aucun déploiement autorisés pour ce lot.
- Résultat : voir `docs/audit-public-sites-20260926.md` pour la matrice des 22 constats, les décisions métier et les limites. Prix sur photos/inspection/accord harmonisés ; réseau en construction ; CTA Fine Bindery vers projet ; accès atelier mobile et liens entre marques ; traductions, liens profonds de langue et SEO corrigés. Pas de 301 racine afin de préserver les retours auth par fragment.
- Validation : typecheck et build Ma Reliure verts ; 226 fichiers / 3 067 tests verts avec un worker, puis 447 tests ciblés après les derniers correctifs ; lint ciblé sans erreur. Recette locale sur origines neuves, bureau/mobile, cinq accueils, parcours allemand, menus/accès atelier, profils/récapitulatifs avec fixtures retirées. Aucune demande envoyée ni donnée production supprimée. Captures et journaux locaux dans output/audit-20260926, non committés.
- Limites : pas de validation d’envoi ou d’inscription de bout en bout, pas de téléphone physique, pages légales FB encore anglaises (signalées), anciens chemins sécurisés/historiques à revoir pour leur marque. Aucune fusion ni publication dans cette mission ; relire la PR avant autorisation de déploiement.

- Reprise PR #50 : périmètre annoncé avant code — conditionner les invitations de l’accueil Fine Bindery à la présence réelle de profils publiés, y compris racine et cinq langues ; vérifier navigation/footer, préserver SEO vide, tests et recette bureau/mobile. Même branche, sans fusion ni déploiement.
- Correctif annuaire vide livré sur la même branche : les quatre liens de découverte du contenu et la section à deux parcours sont conditionnels, alimentés par la liste publique réelle au chargement de `/`, `/{locale}` et `/fine-bindery`. Menu/footer de l’accueil vide pointent vers la section réseau ; avec au moins un profil ils retrouvent l’annuaire. Aucun changement aux règles noindex/sitemap.
- Recette du complément : vingt vues (deux états, cinq langues, bureau/mobile), pas de débordement ; clic du menu mobile vers la section réseau ; huit réponses HTML 200 et SEO vide conservé. Fixture locale retirée, aucune écriture de production. Sept tests ajoutés et verts. Typecheck final, lint et build verts. Suite complète : 3 070 réussites et quatre timeouts de parcours de fichiers ; trois résolus à la relance, un timeout local persistant dans workerPdfLibContract (lecture du bundle). Aucun délai assoupli ; contrôle du bundle intégré au build vert. Vérifier la CI du nouveau SHA avant toute fusion. Sans merge ni déploiement.
