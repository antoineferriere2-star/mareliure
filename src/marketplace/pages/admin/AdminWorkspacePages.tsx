...
export function AdminWorkshopsPage() {
  ...
  return <div className="space-y-5"><h1 className="font-serif text-2xl">Ateliers</h1>
    ...
    <ul className="grid gap-3 md:grid-cols-2">{rows.data?.map((row) => <li key={row.id}><Link to="/admin/ateliers/$binderId" params={{ binderId: row.id }} className={`${CARD} block hover:border-foreground/40`}><span className="flex items-start justify-between gap-3"><strong className="font-serif text-lg">{row.name}</strong><span className={`border px-2 py-1 text-[0.62rem] font-semibold uppercase tracking-[0.1em] ${row.publicProfileStatus === "published" ? "border-emerald-700 text-emerald-800" : "border-amber-700 text-amber-800"}`}>{row.publicProfileStatus === "published" ? "Profil publié" : "Profil non publié"}</span></span><span className="mt-1 block text-sm text-muted-foreground">{row.relieur} · {row.city || "Ville non renseignée"} · {row.countryCode} · {row.status}</span><span className="mt-2 block text-sm">{row.quoteCount} devis · {row.workCount} ouvrages · {row.invoiceCount} factures · {row.portfolioCount} réalisation(s)</span><span className="mt-2 block text-xs text-muted-foreground">{row.specialties.map(binderSkillLabel).join(" · ") || "Aucune spécialité déclarée"}</span><time className="mt-1 block text-xs text-muted-foreground" dateTime={row.lastActivity}>Dernière activité : {new Date(row.lastActivity).toLocaleString("fr-FR")}</time></Link></li>)}</ul>
  </div>;
}
