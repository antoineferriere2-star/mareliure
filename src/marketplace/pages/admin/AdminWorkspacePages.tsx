...
export function AdminWorkshopsPage() {
  ...
  return <div className="space-y-5"><h1 className="font-serif text-2xl">Ateliers</h1>
    ...
    <ul className="grid gap-3 md:grid-cols-2">{rows.data?.map((row) => <li key={row.id}><Link to="/admin/ateliers/$binderId" params={{ binderId: row.id }} className={`${CARD} block hover:border-foreground/40`}><strong className="font-serif text-lg">{row.name}</strong><span className="mt-1 block text-sm text-muted-foreground">{row.relieur} · {row.city || "Ville non renseignée"} · {row.status}</span><span className="mt-2 block text-sm">{row.leadCount} leads Ma Reliure · {row.quoteCount} devis · {row.workCount} ouvrages · {row.invoiceCount} factures</span><time className="mt-1 block text-xs text-muted-foreground" dateTime={row.lastActivity}>Dernière activité : {new Date(row.lastActivity).toLocaleString("fr-FR")}</time></Link></li>)}</ul>
  </div>;
}
...
export function AdminWorkshopPage({ binderId }: { binderId: string }) {
  ...
  return <div className="space-y-5">
    ...
    {tab === "Aperçu" && <div className={CARD}><p>{data.leads.length} leads Ma Reliure · {data.works.length} ouvrages liés · {data.quotes.length} devis liés · {data.invoices.length} factures liées.</p><p className="mt-2 text-sm text-muted-foreground">Clients personnels : {data.privateCounts.works} ouvrages, {data.privateCounts.quotes} devis, {data.privateCounts.invoices} factures. Seuls ces totaux sont visibles ici.</p></div>}
