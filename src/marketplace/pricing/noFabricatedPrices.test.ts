...
function suspiciousAmounts(source: string): string[] {
  const withoutPolicy = code(source)
    .replace(
      /\b(?:targetMarginBps|minimumMarginBps|minimumMarginCents|minimumContributionCents|roundingIncrementCents|depositPercentageBps|depositMinimumCents|DRIFT_\w+|HIGH_DISPERSION_BPS|STALE_AFTER_DAYS)\s*[:=]\s*[\d_]+/g,
      "",
    )
    .replace(/10_000/g, "");
