...
function GenericFormMockup({ locale }: { locale: SupportedLocale }) {
  const copy = (text: string) => publicCopy(locale, text);

  return (
    <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
      <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">
        {copy("Generic website form")}
      </p>
      <div className="mt-4 space-y-4">
        <div>
          <label className="block text-xs font-medium text-slate-500">{copy("Name")}</label>
          <div className="mt-1 rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700">
            Jane Miller
          </div>
        </div>
        <div>
          <label className="block text-xs font-medium text-slate-500">{copy("Email")}</label>
          <div className="mt-1 rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700">
            jane.miller@example.com
          </div>
        </div>
        <div>
          <label className="block text-xs font-medium text-slate-500">{copy("Message")}</label>
...
export function BeforeAfterSection() {
  ...
  const copy = (text: string) => publicCopy(locale, text);
  const points = HONEST_DISCOVERY_POINTS.map((point) => ({
    ...point,
    title: copy(point.title),
    text: copy(point.text),
  }));
  ...
  return (
    <section className="px-4 py-16 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        <SectionHeader
          eyebrow={copy("Before / after")}
          title={copy("Same inquiry. A structured Project Brief instead of a blank message.")}
          description={copy(
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-2 lg:items-start">
          <GenericFormMockup locale={locale} />
