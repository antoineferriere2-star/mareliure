...
const SECTION_TITLES: Record<
  Exclude<keyof ProjectBrief, "generatedAt" | "missionName" | "status" | "projectSummary" | "confidence" | "suggestedNextAction">,
  string
> = {
...
export function BriefPreview({ brief, compact = false }: { brief: ProjectBrief; compact?: boolean }) {
  ...
  return (
    <article className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.14em] text-emerald-700">{brief.status}</p>
        ...
        <div className="flex items-center gap-2">
          <span className={`rounded-full border px-3 py-1 text-xs font-semibold ${CONFIDENCE_STYLE[brief.confidence.label]}`}>
            Confidence: {brief.confidence.label} ({brief.confidence.score})
          </span>
      ...
      {lines.length === 0 ? (
        ...
        <div className="mt-3 space-y-4">
          {Array.from(byCategory.entries()).map(([category, categoryLines]) => (
            <div key={category ?? "_"}>
              {category && <p className="mb-1 text-xs font-semibold uppercase tracking-[0.1em] text-slate-500">{category}</p>}
