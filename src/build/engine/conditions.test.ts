...
describe("evaluateCondition", () => {
  ...
  it("equals / not_equals compare scalar values", () => {
    expect(
      evaluateCondition(
        { fieldKey: "a", operator: "equals", value: "Not sure" },
        { a: "Not sure" },
      ),
    ).toBe(true);
    expect(
      evaluateCondition({ fieldKey: "a", operator: "not_equals", value: "Not sure" }, { a: "Yes" }),
    ).toBe(true);
  });

  it("includes / not_includes check array membership", () => {
    expect(
      evaluateCondition({ fieldKey: "a", operator: "includes", value: "x" }, { a: ["x", "y"] }),
    ).toBe(true);
    expect(
      evaluateCondition({ fieldKey: "a", operator: "not_includes", value: "x" }, { a: ["y"] }),
    ).toBe(true);
    expect(
      evaluateCondition({ fieldKey: "a", operator: "includes", value: "x" }, { a: "not-an-array" }),
    ).toBe(false);
  });

  it("numeric comparisons only apply to numbers", () => {
    expect(
      evaluateCondition({ fieldKey: "a", operator: "greater_than", value: 5 }, { a: 10 }),
    ).toBe(true);
    expect(
      evaluateCondition({ fieldKey: "a", operator: "greater_than", value: 5 }, { a: "10" }),
    ).toBe(false);
  ...
  it("a __NOT_SURE__ answer is never empty but never comparable either", () => {
    expect(evaluateCondition({ fieldKey: "a", operator: "is_empty" }, { a: "__NOT_SURE__" })).toBe(
      false,
    );
    expect(
      evaluateCondition(
        { fieldKey: "a", operator: "equals", value: "__NOT_SURE__" },
        { a: "__NOT_SURE__" },
      ),
    ).toBe(false);
  });
...
describe("evaluateConditionGroup", () => {
  ...
  it("combines all (AND) and any (OR) conjunctively", () => {
    const group = {
      ...
      any: [
        { fieldKey: "b", operator: "equals" as const, value: "x" },
        { fieldKey: "b", operator: "equals" as const, value: "y" },
      ],
    };
