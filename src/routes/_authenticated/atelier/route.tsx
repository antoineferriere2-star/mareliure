...
const PRIMARY_NAV = [
  { to: "/atelier", label: "Aujourd'hui", icon: House, exact: true },
  { to: "/atelier/leads", label: "Leads", icon: Inbox, exact: false },
  { to: "/atelier/messages", label: "Messages", icon: MessageSquare, exact: false },
  { to: "/atelier/devis", label: "Devis", icon: FileText, exact: false },
] as const;
const SECONDARY_NAV = [
  { to: "/atelier/ouvrages", label: "Ouvrages", icon: BookOpen },
...
function AtelierLayout() {
  ...
  const badgeFor = (label: string) => label === "Leads" ? newCount : label === "Messages" ? unreadCount : 0;
    ...
    <div className="min-h-screen bg-[#f4efe6] text-[#241a12] lg:grid lg:grid-cols-[15.5rem_minmax(0,1fr)]">
      ...
      <aside className="hidden min-h-screen border-r border-[#d8d0c4] bg-[#fbf8f2] lg:sticky lg:top-0 lg:flex lg:h-screen lg:flex-col">
        <div className="border-b border-[#d8d0c4] px-6 py-7">
          <Link to="/atelier" className="block font-editorial text-2xl leading-none tracking-[-0.02em]">Ma Reliure</Link>
          <p className="mt-2 text-[0.68rem] font-semibold uppercase tracking-[0.2em] text-[#7a2230]">Espace relieur</p>
        </div>
        <nav aria-label="Espace atelier" className="flex-1 overflow-y-auto px-3 py-5">
          <p className="px-3 pb-2 text-[0.65rem] font-semibold uppercase tracking-[0.16em] text-[#8b8175]">À traiter</p>
          <div className="space-y-1">
            {PRIMARY_NAV.map((item) => { const Icon = item.icon; return (
              <Link key={item.to} to={item.to} activeOptions={item.exact ? { exact: true } : undefined} className="flex min-h-11 items-center gap-3 rounded-sm px-3 text-sm font-medium text-[#5d5146] transition hover:bg-[#eee7dc] hover:text-[#241a12]" activeProps={{ className: "bg-[#e9e0d3] text-[#241a12] shadow-[inset_3px_0_0_#7a2230]" }}>
                <Icon aria-hidden="true" className="h-[1.05rem] w-[1.05rem]" /><span>{item.label}</span><Count value={badgeFor(item.label)} />
              </Link>
            ); })}
          </div>
          <p className="mt-7 px-3 pb-2 text-[0.65rem] font-semibold uppercase tracking-[0.16em] text-[#8b8175]">Mon atelier</p>
          <div className="space-y-1">
            {SECONDARY_NAV.map((item) => { const Icon = item.icon; return (
              <Link key={item.to} to={item.to} className="flex min-h-11 items-center gap-3 rounded-sm px-3 text-sm font-medium text-[#5d5146] transition hover:bg-[#eee7dc] hover:text-[#241a12]" activeProps={{ className: "bg-[#e9e0d3] text-[#241a12] shadow-[inset_3px_0_0_#7a2230]" }}>
                <Icon aria-hidden="true" className="h-[1.05rem] w-[1.05rem]" />{item.label}
              </Link>
            ); })}
          </div>
        </nav>
        <div className="border-t border-[#d8d0c4] px-4 py-3"><SignOutButton label="Se déconnecter" signedInAs="Connecté en tant que" className="inline-flex min-h-11 items-center text-xs font-medium text-[#685d51] underline-offset-4 hover:underline" /></div>
      </aside>
      ...
      <div className="min-w-0">
        <header className="sticky top-0 z-40 flex min-h-16 items-center justify-between border-b border-[#d8d0c4] bg-[#fbf8f2]/95 px-4 backdrop-blur lg:hidden">
          <div><Link to="/atelier" className="font-editorial text-xl leading-none">Ma Reliure</Link><p className="mt-1 text-[0.58rem] font-semibold uppercase tracking-[0.18em] text-[#7a2230]">Espace relieur</p></div>
          <SignOutButton label="Quitter" signedInAs="Connecté en tant que" className="inline-flex min-h-11 items-center px-2 text-xs font-semibold underline-offset-4 hover:underline" />
        </header>
        <main id="atelier-main" className="mx-auto min-h-[calc(100vh-4rem)] max-w-[76rem] px-4 py-7 pb-28 sm:px-6 sm:py-10 lg:px-10 lg:pb-14 xl:px-14">
          {profile.data && profile.data.status !== "approved" && (
            <div role="status" className="mb-7 border-l-4 border-amber-700 bg-amber-50 px-5 py-4 text-sm text-amber-950"><strong className="block">Accès aux leads en attente d'autorisation</strong><p className="mt-1 leading-6">Vous pouvez déjà préparer vos prestations, contacts et devis. Ma Reliure active l'accès aux demandes depuis l'administration.</p></div>
          )}
          <Outlet />
        </main>
      </div>
      ...
      <nav aria-label="Navigation mobile" className="fixed inset-x-0 bottom-0 z-50 grid grid-cols-5 border-t border-[#cfc5b6] bg-[#fffdf8]/95 px-1 pb-[max(0.35rem,env(safe-area-inset-bottom))] pt-1 backdrop-blur lg:hidden">
        {PRIMARY_NAV.map((item) => { const Icon = item.icon; return (
          <Link key={item.to} to={item.to} activeOptions={item.exact ? { exact: true } : undefined} className="relative flex min-h-14 flex-col items-center justify-center gap-1 rounded-sm text-[0.65rem] font-medium text-[#74695d]" activeProps={{ className: "bg-[#f0e9df] text-[#5f1b27]" }}>
            <Icon aria-hidden="true" className="h-[1.1rem] w-[1.1rem]" />{item.label}{badgeFor(item.label) > 0 && <span className="absolute right-[23%] top-1.5 h-2 w-2 rounded-full bg-[#7a2230]" />}
          </Link>
        ); })}
        <details className="group relative">
          <summary className="flex min-h-14 cursor-pointer list-none flex-col items-center justify-center gap-1 rounded-sm text-[0.65rem] font-medium text-[#74695d] [&::-webkit-details-marker]:hidden"><Menu aria-hidden="true" className="h-[1.1rem] w-[1.1rem]" />Plus</summary>
          <div className="absolute bottom-[calc(100%+0.5rem)] right-1 w-52 border border-[#cfc5b6] bg-[#fffdf8] p-2 shadow-xl">
            {SECONDARY_NAV.map((item) => { const Icon = item.icon; return <Link key={item.to} to={item.to} className="flex min-h-11 items-center gap-3 px-3 text-sm hover:bg-[#f0e9df]"><Icon aria-hidden="true" className="h-4 w-4" />{item.label}</Link>; })}
          </div>
        </details>
      </nav>
    </div>
