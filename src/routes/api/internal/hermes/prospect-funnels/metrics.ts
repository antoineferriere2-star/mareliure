...
export async function handleHermesMetrics(request: Request): Promise<Response> {
  ...
  const [
    sessionsRes, dossiersRes, requestsRes, missionsRes,
    membersRes, workspacesRes, rolesRes, viewsRes,
  ] = await Promise.all([
  ...
  for (const v of views) {
    ...
    (seenPerDay.get(key) ?? seenPerDay.set(key, new Set()).get(key)!).add(v.visitor_hash);
  }
  for (const [key, set] of seenPerDay) uniquePerDay[key] = set.size;

  const topPages = Object.entries(countBy(views, (v) => v.path))
    .map(([p, c]) => ({ path: p, label: missionByToken.has(p.slice(3)) ? `Intake · ${missionByToken.get(p.slice(3))}` : p, count: c }))
  ...
  const similarSessionIds = new Set(sessions.map((s) => s.visitor_hash).filter(Boolean));
  const repeatedVisitors = sessions.filter((s) => s.visitor_hash && s.mission_id).length;
  ...
  for (const w of activeSubs) {
    const p = w.plan ?? "unknown";
  ...
  const workspaceActivity = workspaces
    ...
    .map((w) => ({
      ...
      hasStripe: !!w.stripe_subscription_id,
  ...
  const inactivePaying = activeSubs.filter((w) => {
    const wDossiers = dossiers.filter((d) => d.workspace_id === w.id);
    return wDossiers.length === 0;
  });
  ...
  return json(200, {
    ...
    days,
    totals: {
      accounts: 0, // not accessible via service-role REST (Auth API)
    ...
    subscriptions: {
      ...
      byPlan,
      mrrCents,
      mrrUsd: Math.round(mrrCents / 100 * 100) / 100,
      mrrLostCents: mrrLost,
      mrrLostUsd: Math.round(mrrLost / 100 * 100) / 100,
...
export const Route = createFileRoute("/api/internal/hermes/prospect-funnels/metrics")({
  server: { handlers: { GET: async ({ request }) => handleHermesMetrics(request) } },
});