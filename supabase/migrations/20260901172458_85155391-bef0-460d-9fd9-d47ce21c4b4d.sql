DROP FUNCTION IF EXISTS public.publish_workspace_onboarding(UUID, UUID, JSONB, UUID, TEXT);
...
RETURNS TABLE (mission_id UUID, playbook_version_id UUID, reused_existing BOOLEAN)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF p_onboarding_id IS NOT NULL THEN
    SELECT *
      INTO v_onboarding
...
REVOKE ALL ON FUNCTION public.publish_workspace_onboarding(UUID, UUID, JSONB, UUID, TEXT, UUID)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.publish_workspace_onboarding(UUID, UUID, JSONB, UUID, TEXT, UUID)
  TO service_role;