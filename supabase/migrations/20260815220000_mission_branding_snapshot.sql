-- Le branding descend jusqu'au visiteur.
--
-- The setup wizard collects a display name, an accent colour, an intro title,
-- an intro text and a button label. Until now exactly one of them (the button
-- label) reached anyone other than its author: the public runtime received only
-- the workspace name, and the embed snippet was hardcoded to Métré's own green.
-- The customer personalised their intake and nothing came of it.
--
-- This migration gives a published Mission its own frozen copy of the branding.
--
-- Frozen, not joined. The repository already applies this rule to Playbooks:
-- build_playbook_versions is a snapshot, and a published Mission reads the
-- snapshot rather than the draft. Branding must behave the same way — otherwise
-- editing a draft would silently restyle an intake already live on a customer's
-- website, with no publish step and no way for them to see it coming.
--
-- Idempotent and additive. Missions published before this keep working: an
-- empty object means "no branding was captured", and every reader falls back to
...
-- Backfill: Missions already published carry the branding of the setup that
-- produced them. Only fills rows still holding the empty default, so re-running
-- this can never overwrite a snapshot taken since.
...
-- The publish routine now takes the snapshot. Only the INSERT changed; the rest
-- of the body is unchanged from 20260803080534 (lock, idempotency branches,
-- plan limit, version insert, token retry loop).
