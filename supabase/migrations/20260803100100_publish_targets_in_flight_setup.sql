-- Publishing now targets the workspace's in-flight setup rather than "the"
-- setup, since 20260803100000 allows several per workspace (one unpublished,
-- any number published).
--
-- Same signature, so the existing service-role grant and the calling code are
-- unchanged. The only behavioural difference is which row gets locked and
-- published; the plan's max_active_missions check, the version bump and the
-- public-token retry loop are untouched.
...
BEGIN
  ...
  -- A workspace may now hold several setups: at most one in flight (enforced
  -- by build_workspace_onboarding_one_in_flight_uidx) plus any number already
  -- published. Publishing always targets the in-flight one.
  SELECT *
    INTO v_onboarding
  ...
  -- No setup in flight: this is a retry of a publish that already succeeded
  -- (double click, lost response). Fall back to the most recent published row
  -- so the idempotent short-circuit below returns that Mission rather than
  -- raising, or worse, publishing a second one.
  IF NOT FOUND THEN
    SELECT *
      INTO v_onboarding
      FROM public.build_workspace_onboarding
